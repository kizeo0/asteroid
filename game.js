...
const momoSound2 = document.getElementById('momo2.mp3');
...
function setMusicVolume(level, fromSlider = false) {
    ...
    madelineMusic.volume = musicVolume;
    if (fromSlider) { localStorage.setItem('musicVolume', musicVolume); }
}
function setSfxVolume(level, fromSlider = false) {
    sfxVolume = level;
    if (fromSlider) { localStorage.setItem('sfxVolume', sfxVolume); }
}
function playSound(sound) {
    if (!sound) return;
    const clone = sound.cloneNode();
    clone.volume = sfxVolume;
    clone.play().catch(e => { if (!e.message.includes("user interaction")) { console.warn(`Error de sonido: ${e.message}`); } });
}
function loadSettings() {
    ...
    if (savedMusicVol !== null) {
        ...
        setMusicVolume(vol);
        musicVolumeSlider.value = vol * 100;
        musicVolumeLabel.textContent = Math.round(vol * 100);
    }
    if (savedSfxVol !== null) {
        const vol = parseFloat(savedSfxVol);
        setSfxVolume(vol);
        sfxVolumeSlider.value = vol * 100;
        sfxVolumeLabel.textContent = Math.round(vol * 100);
    }
}
...
function spawnNormalEnemy() { if (gamePhase !== 'momo' || (boss && boss.state !== 'leaving')) return; const type = Math.random() < 0.5 ? 'lex' : 'mau'; const edge = Math.floor(Math.random() * 4); const w = canvas.width, h = canvas.height; const margin = ENEMY_SIZE * 1.5; let x, y; if (edge === 0) { x = Math.random() * w; y = -margin; } else if (edge === 1) { x = w + margin; y = Math.random() * h; } else if (edge === 2) { x = Math.random() * w; y = h + margin; } else { x = -margin; y = Math.random() * h; } const angleToCenter = Math.atan2(h / 2 - y, w / 2 - x); const angleVariation = (Math.random() - 0.5) * (Math.PI / 2); const angle = angleToCenter + angleVariation; const speed = ENEMY_SPEED_MIN + Math.random() * (ENEMY_SPEED_MAX - ENEMY_SPEED_MIN) + (level * 0.08); enemies.push({ x: x, y: y, width: ENEMY_SIZE, height: ENEMY_SIZE, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed, type: type, image: enemyImages[type] }); }
...
function checkCollisions() {
    ...
    if (!player.invincible) {
        ...
        if (gamePhase === 'momo') {
             ...
             if (!hitDetected) { for (let i = zomEnemies.length - 1; i >= 0; i--) { if (!zomEnemies[i]) continue; const z = zomEnemies[i]; const dx = player.x - z.x, dy = player.y - z.y; const dist = Math.sqrt(dx * dx + dy * dy); if (dist < (player.width / 2 + z.width / 2) * 0.70) { hitPlayer(); hitDetected = true; break; } } }
        }
