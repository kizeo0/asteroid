...
const powerUpImages = { fast_shot: new Image(), clear_screen: new Image(), ice: new Image() };
...
const zomSound = document.getElementById('zomSound'); // Corregido el ID
...
const POWERUP_COLLECT_DELAY = 3000; // 3 segundos
...
function updatePlayer() {
    ...
    if (keys['KeyZ'] && !isPlayerLocked) {
        ...
        if (lootBox && lootBox.isPlayerNear && lootBox.cooldown <= 0 && gamePhase === 'momo') { openLootBox(); actionConsumed = true; }
...
function checkCollisions() {
    ...
    if (boss && boss.state === 'fighting') {
        for (let i = lasers.length - 1; i >= 0; i--) { if (!lasers[i]) continue; const l = lasers[i]; const dx = l.x - boss.x, dy = l.y - boss.y; const dist = Math.sqrt(dx * dx + dy * dy); if (dist < boss.width / 2 * 0.8) { lasers.splice(i, 1); boss.health--; boss.hitTimer = 5; if(l.type === 'ice') boss.health--; break; } }
...
function updateLootBox() { if (!lootBox) return; if (lootBox.cooldown > 0) { lootBox.cooldown--; if (lootBox.cooldown <= 0) { lootBox.image = lootBoxImg; } } lootBox.isVisible = !boss; if (!lootBox.isVisible) { lootBox.isPlayerNear = false; return; } const dx = player.x - lootBox.x; const dy = player.y - lootBox.y; const distance = Math.sqrt(dx * dx + dy * dy); lootBox.isPlayerNear = distance < LOOT_BOX_PROXIMITY; }
...
function openLootBox() {
    if (!lootBox || lootBox.cooldown > 0 || score < LOOT_BOX_COST) { return; }
    score -= LOOT_BOX_COST;
    updateScoreAndLevel();
    lootBox.cooldown = 180;
    lootBox.image = lootBoxOpenImg;
    playSound(cajaAbiertaSound);
    powerUps.push({
        x: lootBox.x, y: lootBox.y,
        width: POWERUP_SIZE, height: POWERUP_SIZE,
        type: 'ice', image: powerUpImages.ice,
        creationTime: Date.now(),
        collectible: false
    });
    console.log("¡Caja abierta! Ha aparecido un poder de hielo.");
}
